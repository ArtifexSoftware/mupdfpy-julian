...
if ('-h' not in sys.argv and '--help' not in sys.argv
    ...
    if mupdf_local:
        ...
        if platform.system() == 'Windows' or platform.system().startswith('CYGWIN'):
            ...
            if 1:
                unix_build_dir2 = f'build/shared-{unix_build_type}'
                unix_build_dir = f'{mupdf_local}{unix_build_dir2}'
                command = f'cd {mupdf_local} && ./scripts/mupdfwrap.py -d {unix_build_dir2} -b all'
            else:
    ...
    if 0 and mupdf_local and (linux or openbsd or freebsd):
    ...
    if linux or openbsd or freebsd or darwin:
        ...
        if mupdf_local:
            extra_compile_args.append( f'-I{mupdf_local}/platform/c++/include')
    if openbsd:
